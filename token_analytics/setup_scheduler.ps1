...
.DESCRIPTION
    ...
    hora Colombia (COT = UTC-5) todos los días.
...
.USAGE
    # Ejecución básica (detecta rutas automáticamente)
    .\setup_scheduler.ps1
...
if (-not $RepoPath) {
    # Asumir que el script está en token_analytics/ dentro del repo
    $RepoPath = Split-Path (Split-Path $PSScriptRoot -Parent) -Parent
    # Fallback: directorio donde está el script
...
if (-not $ghToken) {
    ...
    Write-Host "El cierre diario no podrá commitear a GitHub sin este token." -ForegroundColor Yellow
    Write-Host "Configúralo con:"-ForegroundColor Cyan
